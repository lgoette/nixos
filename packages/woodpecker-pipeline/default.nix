# nix build .#woodpecker-pipeline && cat result| jq '.configs[].data' -r | jq > .woodpecker/build-arm64.yml
# nix build .#woodpecker-pipeline && cat result| jq '.configs[].data' -r | jq > .woodpecker/build-amd64.yml
